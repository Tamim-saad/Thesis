"""
Tetrahedral Mesh v4: Enhanced Template Deformation Network for Bone FEA
=======================================================================
Builds on v3.2 with 7 targeted accuracy improvements:
  1. FPS interior sampling → cleaner template, more consistent targets
  2. Distance-weighted k-NN → better local feature aggregation
  3. EMA model weights → reduced generalization gap (2.56× → ~1.8×)
  4. Anisotropic scale augmentation → simulates bone proportion variation
  5. 3 iterative refinement steps (was 2) → coarse+medium+fine
  6. Reduced disp_reg (0.005 vs 0.01) → less penalty on necessary displacements
  7. Reduced material_weight (0.05 vs 0.1) → more gradient budget for geometry

V3.2 baseline: 71.3% mean improvement over template (2 folds).
V4 target: 80%+ through generalization gap reduction.
"""
...
print('=' * 60)
print('🦴 Tetrahedral Mesh v4: Enhanced Template Deformation')
print('=' * 60)
...
MODEL_CONFIG = {
    ...
    'encoder_dim': 256,          # full surface feature capacity
    'global_dim': 512,           # encoder_dim * 2
    'hidden_dim': 256,           # full decoder capacity
    'batch_size': 4,
    'epochs': 300,
    'lr': 5e-4,
    'lr_patience': 25,
    ...
    'material_weight': 0.05,     # V4: halved (was 0.1) — more gradient for geometry
    'disp_reg_weight': 0.005,    # V4: halved (was 0.01) — less penalty on needed displacements
    'smooth_reg_weight': 0.005,  # displacement smoothness between neighbors
    'disp_scale': 0.3,           # Output range ±0.3 per refinement step
    'k_folds': 5,
    'early_stop_patience': 50,
    'dgcnn_k': 20,
    'encoder_freeze_epoch': 0,
    'mixup_alpha': 0.4,
    'mixup_prob': 0.0,           # DISABLED — harmful for regression
    'jitter_std': 0.005,         # surface noise std
    'point_dropout': 0.1,        # fraction of surface points to randomly drop
    'n_refine_steps': 3,         # V4: 3 steps (was 2) — coarse+medium+fine
    'ema_decay': 0.999,          # V4: EMA weight averaging for generalization
    'aniso_scale_range': 0.08,   # V4: per-axis scale variation ±8%
    'grad_accum_steps': 2,       # V4: effective batch = 4 * 2 = 8
    'warmup_epochs': 10,         # V4: linear LR warmup
}
...
def farthest_point_sample(pts, n):
    """V4: Deterministic FPS — maximally covers the point cloud.
    Unlike random sampling, FPS ensures consistent spatial coverage
    across all samples, making the mean template MUCH cleaner.
    O(n_pts * n_select) with numpy vectorization.
    """
    m = len(pts)
    if m <= n:
        # Need to pad: FPS all, then random duplicate
        idx = np.arange(m)
...
def process_mesh(nodes, tets, meta, n_surf, n_int):
    ...
    # Interior: xyz + material — V4: use FPS for deterministic, consistent coverage
    fps_idx = farthest_point_sample(int_norm, n_int)
    int_sampled = int_norm[fps_idx]
    mat_sampled = int_mat_raw[fps_idx]
...
class TemplateDeformDataset(Dataset):
    ...
    def _save_material_norm(self):
        ...
        norm_path = os.path.join(OUTPUT_DIR, 'v4_material_norm.npy')
...
class TemplateDeformNet(nn.Module):
    ...
    def __init__(self, global_dim=512, local_dim=256, hidden=256, k_local=8):
        ...
        # V4: Residual displacement head with skip connection
        # Research shows residual MLPs generalize better on small datasets
        self.disp_fc1 = nn.Sequential(
            nn.Linear(inp, hidden), nn.ReLU(), nn.Dropout(0.3))
        self.disp_fc2 = nn.Sequential(
            nn.Linear(hidden, hidden), nn.ReLU(), nn.Dropout(0.2))
        self.disp_skip = nn.Linear(inp, hidden)  # skip connection: project inp→hidden
    ...
    def _get_local_features(self, template, surf_xyz, point_feat):
        ...
        Closer surface points contribute MORE — inverse distance weighting.
        """
        ...
        for i in range(0, T, chunk):
            ...
            topk_dist, nn_idx = dist.topk(k, dim=2, largest=False)  # (B, c, k)
            # Gather features
            ...
            # V4: Inverse distance weights (softmax for stability)
            weights = 1.0 / (topk_dist + 1e-8)  # (B, c, k)
            weights = weights / weights.sum(dim=-1, keepdim=True)  # normalize
            local = (gathered * weights.unsqueeze(-1)).sum(dim=2)  # (B, c, D)
    ...
    def forward(self, template, surf_xyz, global_feat, point_feat):
        ...
        # V4: Residual displacement prediction
        h = self.disp_fc1(node_input)
        h = self.disp_fc2(h) + self.disp_skip(node_input)  # skip connection
        disp = self.disp_out(h)                             # (B, T, 3)
...
# ============================================================
class EMAModel:
    """V4: Exponential Moving Average of model weights.
    Maintains a smoothed copy of weights that generalizes better.
    Used for evaluation; training uses the original model.
    """
...
class Trainer:
    def __init__(self, model, train_dl, val_dl, fold=1):
        ...
        # V4: EMA for generalization
        self.ema = EMAModel(model, decay=MODEL_CONFIG.get('ema_decay', 0.999))
        print(f"    📊 EMA enabled (decay={MODEL_CONFIG.get('ema_decay', 0.999)})")
    ...
    def _run_epoch(self, dl, train=True):
        ...
        for surf, template, target_pos, target_mat, _ in dl:
            ...
            if train:
                ...
                # V4: Gradient accumulation — effective batch = batch_size * accum_steps
                accum_steps = MODEL_CONFIG.get('grad_accum_steps', 1)
                loss_scaled = loss / accum_steps
                loss_scaled.backward()
                if (n // surf.size(0) + 1) % accum_steps == 0:
                    nn.utils.clip_grad_norm_(self.model.parameters(), 1.0)
                    self.opt.step()
                    self.ema.update(self.model)  # V4: update EMA after each step
                    self.opt.zero_grad()
            bs = surf.size(0)
    ...
    def train(self):
        ...
        for ep in range(1, MODEL_CONFIG['epochs'] + 1):
            # V4: Linear LR warmup for first N epochs
            ...
            t0 = time.time()
            self.opt.zero_grad()  # V4: zero grad at start (for gradient accumulation)
            ...
            if accum_steps > 1:
                ...
                self.opt.zero_grad()
            with torch.no_grad():
                # V4: Use EMA weights for validation
            ...
            # V4: Only use ReduceLROnPlateau AFTER warmup
            if ep > warmup_epochs:
                self.scheduler.step(val_cd)
        ...
        print(f"  ✅ Fold {self.fold} done | Best val CD: {best_val:.6f} (EMA)")
        return history
...
class _SubDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # V4: Anisotropic scale — per-axis stretch simulates bone proportion variation
            aniso_range = MODEL_CONFIG.get('aniso_scale_range', 0.08)
            scale_xyz = (1.0 + np.random.uniform(-aniso_range, aniso_range, 3)).astype(np.float32)
            surface[:, :3] = (surface[:, :3] @ R.T) * scale_xyz
            surface[:, 3:] = surface[:, 3:] @ R.T
            # V4 FIX: Correct normals under anisotropic scale
            # Normals transform by inverse-transpose of scale: n_new = n / scale_xyz
            surface[:, 3:] = surface[:, 3:] / scale_xyz
            nrm = np.linalg.norm(surface[:, 3:], axis=1, keepdims=True)
            surface[:, 3:] /= np.maximum(nrm, 1e-10)
            template = (template @ R.T) * scale_xyz
            target_pos = (target_pos @ R.T) * scale_xyz
...
def run_kfold(meshes):
    print("\n" + "=" * 60)
    print("🧠 K-FOLD CROSS-VALIDATION v4 (Enhanced Template Deformation)")
    print("=" * 60)
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(range(n), groups=groups)):
        ...
        tpl_path = os.path.join(OUTPUT_DIR, f'v4_template_fold{fold+1}.npy')
        ...
        save_path = os.path.join(OUTPUT_DIR, f'model_v4_fold{fold+1}.pt')
...
class ResultsViz:
    ...
    def training_curves(fold_results):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, 'v4_training_curves.png'), dpi=150, bbox_inches='tight')
...
def run_pipeline(skip_training=False):
    print("\n" + "=" * 70)
    print("🦴 V4 Enhanced Template Deformation — Bone FEA Mesh Generation")
    print("   DGCNN + Distance-weighted k-NN + FPS + EMA + 3-step Refinement")
    print("=" * 70)
