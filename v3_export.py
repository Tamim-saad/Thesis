"""
V3 Export Script: Template Deformation Model → CDB Files
=========================================================
Loads trained V3 model, predicts interior points + materials,
runs TetGen for tet meshing, writes ANSYS CDB files.

Usage (Colab):
  exec(open('/content/drive/MyDrive/thesis/v3_export.py').read())

Usage (local):
  python v3_export.py --model model_v3_fold3.pt --data ./4_bonemat_cdb_files --output ./v3_output/
"""
...
class TemplateDeformNet(nn.Module):
    ...
    def _get_local_features(self, template, surf_xyz, point_feat):
        ...
        for i in range(0, T, chunk):
            ...
            _, nn_idx = dist.topk(k, dim=2, largest=False)
            ...
            local_feats.append(gathered.mean(dim=2))
    ...
    def forward(self, template, surf_xyz, global_feat, point_feat):
        ...
        disp = self.disp_head(node_input)
...
def run_export(model_path=None, data_dir=None, output_dir=None):
    if IN_COLAB:
        model_path = model_path or '/content/drive/MyDrive/thesis/thesis_output/model_v3_fold3.pt'
        data_dir = data_dir or '/content/drive/MyDrive/thesis/4_bonemat_cdb_files'
        output_dir = output_dir or '/content/drive/MyDrive/thesis/thesis_output/v3_export/'
    else:
        model_path = model_path or './thesis_output/model_v3_fold3.pt'
        data_dir = data_dir or './4_bonemat_cdb_files'
        output_dir = output_dir or './thesis_output/v3_export/'

    os.makedirs(output_dir, exist_ok=True)

    print("\n" + "=" * 60)
    print("V3 TEMPLATE DEFORMATION — EXPORT")
    print("=" * 60)
    ...
    if fold_num:
        fold_tpl = os.path.join(model_dir, f'v3_template_fold{fold_num}.npy')
    ...
    if template is None:
        global_tpl = os.path.join(model_dir, 'v3_template.npy')
    ...
    if template is None:
        ...
        if all_interiors:
            ...
            tpl_save = os.path.join(model_dir, 'v3_template.npy')
    ...
    if gmin is None:
        ...
        norm_path = os.path.join(os.path.dirname(model_path), 'v3_material_norm.npy')
    ...
    for fi, fp in enumerate(cdb_files):
        ...
        try:
            ...
            # 10. Write CDB
            out_path = os.path.join(output_dir, f'v3_{name}')
