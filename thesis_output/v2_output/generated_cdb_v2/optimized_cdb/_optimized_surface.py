...
def batch_process(cdb_dir, output_dir):
    """Process all fixed_*.cdb files in a directory."""
    pattern = os.path.join(cdb_dir, 'fixed_*.cdb')
    files = sorted(glob.glob(pattern))

    if not files:
        # Fallback: try generated_*.cdb
        pattern = os.path.join(cdb_dir, 'generated_*.cdb')
        files = sorted(glob.glob(pattern))

    if not files:
        print(f"[ERROR] No fixed_*.cdb or generated_*.cdb files found in: {cdb_dir}")
        return []
...
if __name__ == "__main__":
    ...
    # CDB files are in the parent directory
    cdb_dir = os.path.dirname(script_dir)
    # Save output in this directory (optimized_cdb/)
